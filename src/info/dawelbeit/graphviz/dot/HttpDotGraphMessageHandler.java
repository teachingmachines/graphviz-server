...
//			log.info("Incoming entity content (bytes): " + entityContent.length);
        ...
        byte[] dotBytes = null;
        try {
            dot = (StringUtils.isNotBlank(target) ?
                    new URLCodec().decode(new URI(target).getQuery()) : "");
            dotBytes = new URLCodec().decode(new URI(target).getQuery().getBytes());
        ...
        if(StringUtils.isNotBlank(dot) && GraphViz.isValidDotText(dot)) {
            ...
            target = (StringUtils.isNotBlank(target) ?
                    StringUtils.remove((URLDecoder.decode(target, "UTF-8")), '/') : null);
        ...
        dot = Gzip.decompress(Base64.decodeBase64(dotBytes));
        if(StringUtils.isNotBlank(dot) && GraphViz.isValidDotText(dot)) {
